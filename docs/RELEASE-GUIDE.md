...
- `[inserire email]` → la tua email di contatto
- `[inserire email security]` → email per security disclosure (può essere la stessa)
...
grep -rl "\[inserire email\]" . | xargs sed -i '' 's/\[inserire email\]/tua@email.it/g'
```
